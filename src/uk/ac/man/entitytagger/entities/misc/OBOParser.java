...
public class OBOParser {
	...
	public static void main(String[] args) {
		...
		run(inFiles, outFile, invalidParentsFile, invalidParentsRecursiveFile, logger, pstmt);	
	}

	private static void run(File[] inFiles, File outFile, File invalidParentsFile, File invalidParentsRecursiveFile, Logger logger, PreparedStatement pstmt) {
		try{
			...
			for (File file : inFiles){
				...
				for (String line : fileContents){
					if (line.equals("[Term]")){
						...
						if (!isInvalid){
							names = regexpify(names);
							filterNames(names);

							if (id != null && names.size() > 0){
								DictionaryEntry de = new DictionaryEntry("OBO:" + id, Misc.implode(parents.toArray(new String[0]), "|"));
	...
	private static List<String> regexpify(List<String> names) {
		List<String> res = new ArrayList<String>();
		for (String name : names)
			res.add(regexpify(name));
		return res;
	}
	...
	private static void filterNames(List<String> names) {
		for (int i = 0; i < names.size(); i++)
			if (names.get(i) == null || names.get(i).length() < 4)
				names.remove(i--);
	}

	private static String regexpify(String str) {
		if (str.length() == 0)
			return null;
		if (str.length() < 4)
			return null;
