	...
	public static void main(String[] args) {
		...
		if (ap.containsKey("species") || ap.containsKey("genus") || ap.containsKey("regexp")){
			...
			if (ap.containsKey("species"))
				dict = uk.ac.man.entitytagger.entities.species.GenerateDictionary.generateSpeciesDictionary(ap.getFile("species"), extraSynonymFiles, includeLineNumbers, logger);
	...
	private static void convertToVariants(
		...
		try{
			...
			for (Tuple<DictionaryEntry,Set<String>> t : master){
				if (outStream != null){
					String s = t.getA().getId() + "\t" + Misc.implode(t.getB(), "|") + "\t";
