...
public class EntityTagger {
	public static final String LINNAEUS_VERSION = "LINNAEUS, v. 1.5 (May 11th, 2010)";
	...
	public static Postprocessor getPostprocessor(ArgParser ap, Logger logger, Map<String,String> comments, String tag){
		if (ap.containsKey("postProcessing" + tag)){
			File[] stop = ap.getFiles("ppStopTerms" + tag);
			File[] acr = ap.getFiles("ppAcrProbs" + tag);
			File[] spf = ap.getFiles("ppEntityFreqs" + tag);

			return new Postprocessor(stop,acr,spf,comments,logger);
		} else {	
			if (logger != null)
	...
	public static Matcher getMatcher(ArgParser ap, Logger logger, String tag){
		...
		if (ap.containsKey("variantMatcher" + tag)){
			...
			for (File f : ap.getFiles("variantMatcher" + tag)){
				logger.info("%t: Loading variantMatcher from " + f.getAbsolutePath() + ", ignoreCase = " + ignoreCase + "...\n");
				matchers.add(VariantDictionaryMatcher.load(f, ignoreCase));
			}
		}
		...
		Matcher matcher = matchers.size() == 1 ? matchers.get(0) : new MultiMatcher(matchers, true);
	...
	public static void main(String[] args) {
		...
		if (ap.containsKey("outDir")){
			...
			MatchOperations.run(matcher,documents, numThreads, report, ap.getFile("outDir"),logger);
		}
		...
		if (ap.containsKey("outDB")){
			...
			Connection dbConn = martin.common.SQL.connectMySQL(ap, logger, "out");
			MatchOperations.runDB(matcher,documents,numThreads,ap.get("outDB"), report, logger, dbConn);
		}
